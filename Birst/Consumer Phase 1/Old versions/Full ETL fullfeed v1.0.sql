...
--ga_leads_native_app
TRUNCATE TABLE appwithlead;
INSERT INTO  appwithlead
SELECT * FROM ga_leads_native_app
WHERE date >= '20160905' 
;

-- ga_leads_no_native_app
TRUNCATE TABLE webwithlead;
INSERT INTO  webwithlead
SELECT * FROM ga_leads_no_native_app
WHERE date >= '20160905' ;

---ga_sessions_no_lead    
TRUNCATE TABLE webandappnolead;
INSERT INTO  webandappnolead
SELECT * FROM ga_sessions_no_lead 
WHERE date >= '20160905' ;

-- ga_visit_platform     
TRUNCATE TABLE visit_platform;
INSERT INTO  visit_platform
SELECT * FROM ga_visit_platform 
WHERE date >= '20160905' ;
...
TRUNCATE TABLE WebWithLeadDeduped;

INSERT INTO public.WebWithLeadDeduped
select *
from 
(
...
--union 3 web queries
truncate  table public.GAData_delta;

Insert into public.GAData_delta
select *
from (
...
from public.WebWithLeadDeduped wl
...
wl.totals_pageviews as TotalPageviews,
wl.totals_newVisits as NewVisits
from public.WebAndAppNoLead wl
...
--truncatepublic.LeadsLink_delta;

TRUNCATE TABLE public.LeadsLink_delta;

Insert into public.LeadsLink_delta
select *
from (
...
inner join public.agent_leads_unique_identifiers i
...
from (
  ...
  --native app with TransactionID
	...
	wl.visitId as VisitID,
	wl.hits_transaction_transactionId as TransactionID
	from public.AppWithLead wl	where len(wl.hits_transaction_transactionId)<32
	group by cast(cast(left(wl.date,4) as char(4))+'-'+substring(cast(wl.date as char(8)),5,2)+'-'+cast(right(wl.date,2) as char(4)) as date),
	wl.fullVisitorId,
	wl.visitId,
	wl.hits_transaction_transactionId
...
--inner join public.agent_leads_unique_identifiers i  --dropped as not relevant for older lead IDs
--	on wl.TransactionID=i.lead_id
group by 1,2,3,4,5,6
				)d;

truncate public.ZPGLeads_delta;

Insert into public.ZPGLeads_delta
select *
from (
...
from public.source_listing_agent_leads_nopii sl
where (sl.sent='Y' OR sl.sent='N')
and (sl.sent_date between  '09/05/2016' and '09/12/2016')
...
from public.agent_leads_nopii al
inner join public.agent_leads_sent alsent
	on al.lead_id=alsent.lead_id
--lead type not required
where al.type_of_lead<> 'temptme'
and (alsent.sent_date between  '09/05/2016' and '09/12/2016')
union all
...
from public.agent_phone_leads_nopii pl
Where pl.call_start between '09/05/2016' and '09/12/2016'
) d;


Truncate public.JoinedDataFinalStaging_delta;

Insert into public.JoinedDataFinalStaging_delta
select
...
from public.GAData_delta g
	left join public.visit_platform p
...
from public.JoinedDataFinalStaging_delta g
